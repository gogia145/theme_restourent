...
class rf_fronttext_widget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array( 
			'classname' => 'rf_fronttext_widget', 
			'description' => 'A widget that displays an image, text and a read-more button.'
		);
		$control_ops = array( 
			'width' => 400, 
			'height' => 250, 
			'id_base' => 'rf_fronttext_widget' 
		);
		parent::__construct( 
			'rf_fronttext_widget', 
	...
	public function widget( $args, $instance ) {
		...
		<div class="rf_fronttext_widget">
			<?php if ($image) { ?>
            <a class="fronttext-image-container" href="<?php echo $url; ?>" title="<?php echo $title; ?>" rel="bookmark" >
                <div class="fronttext-image" style="background-image: url(<?php echo $image; ?>); height: <?php echo $imageheight; ?>px;" alt="<?php echo $title; ?>"></div>
            </a>
            <?php } ?>
           	...
           	<?php if ($url) { ?>
			<a class="btn" href="<?php echo $url; ?>" title="<?php echo $title; ?>" rel="bookmark"><?php _e( 'Read more', 'the-restaurant' ); ?></a>
            <?php } ?>
		</div>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['imagezoom'] = $new_instance['imagezoom'];
	...
	public function form( $instance ) {
        ...
        <p>
        	<label for="<?php echo $this->get_field_id( 'imagezoom' ); ?>">Enlarge image on click:</label>
        	<input class="" type="checkbox" <?php if ($instance['imagezoom'] == '1') { echo 'checked'; } ?> id="<?php echo $this->get_field_id( 'imagezoom' ); ?>" name="<?php echo $this->get_field_name( 'imagezoom' ); ?>" value="1" />
        </p>
        <p>
